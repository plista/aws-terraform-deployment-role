resource "aws_iam_role" "SQUAD_my_software_deployment_role" {
  ...
  name = "${local.prefixes["squad_my_software"]}-deployment-role-${local.suffixes[count.index]}"
  description = "Role for ${local.prefixes["squad_my_software"]} ${local.suffixes[count.index]}"
